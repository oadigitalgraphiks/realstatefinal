...
class CheckoutController extends Controller
{
    ...
    public function address_store(Request $request)
    {
        ...
        if(isset($request->ship_to_different_address)){
            ...
            $address_shipping->first_name    = $request->shipping_first_name;
            $address_shipping->last_name     = $request->shipping_last_name;
            $address_shipping->email         = $request->shipping_email;
            $address_shipping->address       = $request->shipping_address;
            $address_shipping->country_id    = $request->shipping_country_id;
            $address_shipping->state_id      = $request->shipping_state_id;
            $address_shipping->city_id       = $request->shipping_city_id;
            $address_shipping->longitude     = $request->longitude;
            $address_shipping->latitude      = $request->latitude;
            $address_shipping->postal_code   = $request->shipping_billing_postcode;
            $address_shipping->phone         = $request->shipping_phone;
        ...
        foreach ($carts as $key => $cartItem) {
            if(isset($request->ship_to_different_address)){
                $cartItem->address_id = $address_shipping->id;
            }else{
        ...
        if ($carts && count($carts) > 0) {
            foreach ($carts as $key => $cartItem) {
                $product = \App\Models\Product::find($cartItem['product_id']);
                $country = \App\Models\Country::find($shipping_info->country_id);
                if ($country->code == 'AE') {
                    $cartItem['tax'] = (($cartItem['price'] / 100) * 5);
                    $tax += (($cartItem['price'] / 100) * 5) * $cartItem['quantity'];
                }else{
                    $cartItem['tax'] = 0.00;
                    $tax += $cartItem['tax'] * $cartItem['quantity'];
                }
    ...
    public function cart_tax(Request $request)
    {
        ...
        if ($carts && count($carts) > 0) {
            foreach ($carts as $key => $cartItem) {
                    $cartItem->address_id = $request->address_id;
                    $country = \App\Models\Country::where("code",$request->country_code)->first();
                    if ($country != null) {
                        if ($country->code == 'AE') {
                            $cartItem['tax'] = (($cartItem['price'] / 100) * 5);
                        }else{
        ...
        return view("frontend.partials.cart_summary",compact("carts"));
    }
