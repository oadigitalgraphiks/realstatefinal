...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $popular_products = filter_products(Product::where('published', 1)->where('featured', 1)->orderBy('category_id', 'desc'))->limit(12)->get();

        return view('frontend.index', compact('featured_categories', 'todays_deal_products','popular_products'));
    }
    ...
    public function popular_product(Request $request)
    {
        $cat_id = $request->id;
        if ($cat_id != null) {
            $popular_products = filter_products(Product::where('published', 1)->where('featured', 1)->where('category_id', $cat_id))->limit(24)->get();
        } else {
            $popular_products = filter_products(Product::where('published', 1)->where('featured', 1)->orderBy('category_id', 'desc'))->limit(24)->get();
        }
        return view('frontend.partials.products.new_arrival_products', compact('popular_products', 'cat_id'));
    }
...
//        $categories = Category::where('level', 0)->orderBy('name', 'asc')->get();
    ...
    public function get_mob_menus_items(Request $request)
    {
        $menus = Menu::findOrFail($request->id);
        return view('frontend.partials.mob_menus_elements', compact('menus'));
    }
    ...
    public function contact_store(Request $request)
    {
        ...
        $array['subject'] = 'User Query';
        $array['from'] = $conatct->email;
        $array['contact'] = $conatct;
        try {
        ...
        flash("Succefully Submit")->success();
        return back();
    }
