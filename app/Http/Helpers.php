...
if (! function_exists('current_curency')) {
    function current_currency()
    {
        $curl = curl_init();

        curl_setopt_array($curl, array(
        CURLOPT_URL => 'http://www.floatrates.com/daily/usd.json',
        ...
        CURLOPT_HTTPHEADER => array(
            'Cookie: PHPSESSID=201hc1vcmdbgqfnvsknimabe55'
        ),
        ));
        ...
        foreach ($currencies as $key => $currency) {
            $curr = strtolower($currency->code);
            $currt = json_decode($response)->$curr ?? "";
            if ($currt != null) {
                if (strtolower($currt->code) == strtolower($currency->code)) {
                    $currnecy_update = Currency::where('status',1)->where('code',$currt->code)->first();
                    $currnecy_update->exchange_rate = $currt->rate;
                    $currnecy_update->save();
                }else{

                    echo $key."- Not match -- ".strtolower($currency->code).'<br>';
                }
            }
        }
        return flash("Successfully Updated Currency")->success();
    }
}
...
if (! function_exists('menu_permissions')) {
    function menu_permissions($menu_id)
    {
        $roleId = Auth::user()->staff->role_id;

        return in_array($menu_id, Role::findOrFail($roleId)->menus->pluck('id')->toArray());
    }
}

if (! function_exists('frontend_permissions')) {
    function frontend_permissions($current_route)
    {
        $roleId = Auth::user()->staff->role_id;

        return in_array($current_route, Role::findOrFail($roleId)->permissions->pluck('route')->toArray());
    }
}
...
if (! function_exists('admin_menus')) {
    ...
    {
        return AdminMenu::with('childrens')->where('parent_id',0)->orderBy('sort','asc')->where('status',1)->get();
    }
}

if (! function_exists('getMenuIdForPermission')) {
    function getMenuIdForPermission($current_route)
    {
        return AdminMenu::where('route',$current_route)->orWhereHas('permissions', function (Builder $query) use($current_route) {
            $query->where('route', $current_route);
        })->first()->id ?? null;
    }
}
...
if(!function_exists('wishlist')){
    function wishlist($product_id){
        $whislist = 0;
        if(Auth::check()){
            $whislist = Wishlist::where('user_id',Auth::user()->id)->where('product_id',$product_id)->count();
        }
        return $whislist;
    }
}
...
if(!function_exists('mergeArrayofArrays')){
    function mergeArrayofArrays($arrays)
    {
        $results = [];
