        ...
        <div class="post d-flex flex-column-fluid" id="kt_post">
            <div id="kt_content_container" class="container-xxl">
                    ...
                    @foreach (\App\Models\Language::all() as $key => $language)
                        <li class="nav-item">
                            <a class="nav-link text-reset @if ($language->code == $lang) active @else bg-soft-dark border-light border-left-0 @endif py-3"
                                href="{{ route('property_type.edit', ['id' => $module->id, 'lang' => $language->code]) }}">
                ...
                <form class="form d-flex flex-column flex-lg-row gap-7 gap-lg-10"
                    action="{{ route('property_type.update', $module->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                        ...
                        <div class="card card-flush py-4">
                            ...
                            <div class="card-body pt-0">
                                ...
                                <div class="mb-5 fv-row">
                                    ...
                                    <input type="text" value="{{$module->getTranslation('name', $lang)}}" placeholder="{{ translate('Name') }}" id="name" name="name" class="title form-control mb-2" required>
                                        @if($errors->has('name'))
                                ...
                                <div class="mb-5 fv-row">
                                    ...
                                    <input type="text" placeholder="{{ translate('slug') }}" id="slug" name="slug" class="slug form-control mb-2" value="{{$module->slug}}" required>
                                    @if($errors->has('slug'))
                                ...
                                <div class="fv-row mb-2">
                                    ...
                                    <select class="form-select mb-2" data-control="select2" data-hide-search="false"
                                        ...
                                        @foreach ($property_types as $property_type)
                                            <option @if($property_type->id == $module->parent_id){{'selected'}} @endif value="{{ $property_type->id }}">{{ $property_type->getTranslation('name') }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="fv-row mb-2">
                                    ...
                                    <div class="dropzone" id="kt_ecommerce_add_product_mediaa"
                                        ...
                                        <div class="dz-message needsclick">
                                            ...
                                            <input type="hidden" name="icon" value="{{ $module->icon }}" class="selected-files">
                                            <div class="ms-4">
                                ...
                                <div class="py-2">
                                    ...
                                    <textarea name="meta_description" rows="3" class="form-control mb-2">{{ $module->meta_description}}</textarea>
                                </div>

                                <div class="py-2"  >
                                    <label class="form-label">{{ translate('Order level')}}</label>
                                    <input type="number" name="order_level" class="form-control mb-2" id="order_level" value="{{ $module->order_level}}" placeholder="{{ translate('Order Level') }}">
                                </div>
