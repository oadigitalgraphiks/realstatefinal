        ...
        <div class="toolbar" id="kt_toolbar">
            <div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
                <div data-kt-swapper="true" data-kt-swapper-mode="prepend"
                    ...
                    <h1 class="d-flex align-items-center text-dark fw-bolder fs-3 my-1"> {{ translate('Property Conditions') }}</h1>
            ...
            <div id="kt_content_container" class="container-xxl">
                <form  method="GET">
                        ...
                        <div class="card-header align-items-center py-5 gap-2 gap-md-5">
                            ...
                            <div class="card-toolbar flex-row-fluid justify-content-end gap-5">
                                <span class="text-muted"> {{ translate('Displaying') }}  
                                    {{ ($property_conditions->currentPage() > 1) ? (($property_conditions->currentPage()-1) * $property_conditions->perPage()) : ((count($property_conditions) > 0) ? 1 : 0) }} -
                                        
                                    {{ ($property_conditions->currentPage() > 1) ? (($property_conditions->currentPage()-1) * $property_conditions->perPage() + count($property_conditions)) : count($property_conditions)}} {{ translate('of') }} <span class="count_show"> {{$property_conditions->total()}} </span> 
                                    {{ translate('Records') }}
                                </span>
                                    ...
                                    <tbody class="fw-bold text-gray-600">
                                        @foreach ($property_conditions as $key => $property_condition)
                                            <tr class="text-center" >
                                                <td class="text-center">{{$property_condition->id}}</td>
                                                <td>{{ $property_condition->getTranslation('name') }}</td>
                                                <td class="text-center">
                                                    ...
                                                    <a href="#" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm confirm-delete" data-href="{{ route('property_conditions.destroy', $property_condition->id) }}">
                    ...
                    <div class="aiz-pagination pt-4">
                        {{ $property_conditions->appends(request()->input())->links() }}
                    </div>
                </form>
            </div>
        </div>
