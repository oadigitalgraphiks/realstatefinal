    ...
    <div class="content d-flex flex-column flex-column-fluid" id="kt_content">
            ...
            <div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
                <div data-kt-swapper="true" data-kt-swapper-mode="prepend"
                    ...
                    <ul class="breadcrumb breadcrumb-separatorless fw-bold fs-7 my-1">
                        ...
                        <li class="breadcrumb-item text-muted"><a href="{{route('property_baths.index')}}" class="text-muted text-hover-primary">{{ translate('Property Beds') }}</a></li>
                    </ul>
                </div>
            </div>
        </div>
        ...
        <div class="post d-flex flex-column-fluid" id="kt_post">
            <div id="kt_content_container" class="container-xxl">
                    ...
                    @foreach (\App\Models\Language::all() as $key => $language)
                        <li class="nav-item">
                            <a class="nav-link text-reset @if ($language->code == $lang) active @else bg-soft-dark border-light border-left-0 @endif py-3"
                                href="{{ route('property_baths.edit', ['id' => $data->id, 'lang' => $language->code]) }}">
                ...
                <br>
                <form class="form d-flex flex-column flex-lg-row gap-7 gap-lg-10" action="{{route('property_baths.update', $data->id) }}" method="POST"  >
                  @csrf
