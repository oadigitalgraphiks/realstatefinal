    ...
    <div class="toolbar" id="kt_toolbar">
        <div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
            <div data-kt-swapper="true" data-kt-swapper-mode="prepend"
                ...
                <h1 class="d-flex align-items-center text-dark fw-bolder fs-3 my-1">{{translate('All Nested Areas')}}</h1>
                ...
                <ul class="breadcrumb breadcrumb-separatorless fw-bold fs-7 my-1">
                    ...
                    <li class="breadcrumb-item text-muted"><a class="text-muted" href="{{route('property_nested_areas.index')}}" >{{translate('Nested Areas')}}</a></li>
                </ul>
            </div>
        </div>
    </div>
                    ...
                    <div class="card-header align-items-center py-5 gap-2 gap-md-5">
                        <div class="card-title">
                            ...
                            <div class="d-flex align-items-center position-relative my-1">
                                ...
                                <input type="text" class="form-control form-control-solid w-250px ps-14" name="sort" @isset($sort) value="{{ $sort}}" @endisset placeholder="{{ translate('Type Name') }}" />
                            </div>
                          </form>
                        </div>
                                ...
                                <tbody class="fw-bold text-gray-600">
                                    @foreach ($data as $key => $item)
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                @if($item->area)
                                                    <a href="{{route('property_areas.edit',$item->area->id)}}">
                                                        {{ $item->area->name}}
                                                    </a>
                                                @else
                                                    -
                                            ...
                                            <td class="text-center">
                                                @if($item->area && $item->area->city)
                                                    <a href="{{route('property_cities.edit',$item->area->city->id)}}">
                                                        {{ $item->area->city->getTranslation('name') }}
                                                    </a>
                                                @else
                                                    -
                                                @endif
                                            ...
                                            <td class="text-center">
                                                @if($item->area && $item->area->city && $item->area->city->state)
                                                    <a href="{{route('property_states.edit',$item->area->city->state->id)}}">
                                                        {{ $item->area->city->state->getTranslation('name') }}
                                                    </a>
                                                @else
                                                    -
                                                @endif
                                            ...
                                            <td class="text-center">
                                                @if($item->area && $item->area->city && $item->area->city->state && $item->area->city->state->country)
                                                    <a href="{{route('property_countries.edit',$item->area->city->state->country->id)}}">
                                                    {{ $item->area->city->state->country->getTranslation('name')}}
                                                    </a>
                                                @else
                                                    -
                                                @endif
                                            ...
                                            <td class="text-center">
                                                <a href="{{ route('property_nested_areas.edit',['id' => $item->id, 'lang' => env('DEFAULT_LANGUAGE')]) }}" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">
                                                <span class="svg-icon svg-icon-3"><i class="text-primary fas fa-marker" ></i></span></a>
                                                
                                                <a data-href="{{ route('property_nested_areas.destroy', $item->id) }}"
                                                    class="confirm-delete btn btn-icon btn-bg-light btn-active-color-primary btn-sm"
                                                    data-href="{{ route('property_nested_areas.destroy', $item->id) }}">
...
@section('script')
    <script type="text/javascript">
          $("table").delegate(".onStatuChange", "change", function(){   
                ...
                let id = $(this).val();
                $.get("{{route('property_nested_areas.bulk')}}",
                { 
                    idz:id,
                    action:'status',
