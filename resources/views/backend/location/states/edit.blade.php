            ...
            <div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
                <div data-kt-swapper="true" data-kt-swapper-mode="prepend"
                    ...
                    <ul class="breadcrumb breadcrumb-separatorless fw-bold fs-7 my-1">
                        ...
                        <li class="breadcrumb-item text-muted"><a href="{{route('property_states.index')}}" >{{translate('States')}}</a></li>
                    </ul>
                </div>
            </div>
        </div>
        ...
        <div class="post d-flex flex-column-fluid" id="kt_post">
            <div id="kt_content_container" class="container-xxl">
                    ...
                    @foreach (\App\Models\Language::all() as $key => $language)
                        <li class="nav-item">
                            <a class="nav-link text-reset @if ($language->code == $lang) active @else bg-soft-dark border-light border-left-0 @endif py-3"
                                href="{{ route('property_states.edit', ['id' => $data->id, 'lang' => $language->code]) }}">
                ...
                <form class="form d-flex flex-column flex-lg-row gap-7 gap-lg-10"
                    action="{{ route('property_states.update', $data->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                            ...
                            <div class="card-body">
                                ...
                                <div class="py-2">
                                    ...
                                    <select required class="form-select mb-2"  data-placeholder="Select an option" name="country_id" id="country_id">
                                        @if($data->country)
                                        <option selected value="{{$data->country_id}}">{{$data->country->name}}</option>
                                        @endif
                                    </select>
                                </div>
