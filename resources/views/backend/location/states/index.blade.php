                ...
                <div class="card card-flush">
                    <div class="card-header align-items-center py-5 gap-2 gap-md-5">
                        ...
                        <div class="card-toolbar flex-row-fluid justify-content-end gap-5">
                            ...
                            <a href="{{ route('property_states.create') }}" class="btn btn-primary" type="submit">{{ translate('Create New') }}</a>
                        </div>
                    </div>
                                ...
                                <tbody class="fw-bold text-gray-600">
                                    @foreach ($data as $key => $item)
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                @if($item->country)
                                                    <a href="{{route('property_countries.edit',$item->country->id)}}">
                                                        {{ $item->country->getTranslation('name') }}
                                                    </a>
                                                @else
                                                    -
                                                @endif
                                            ...
                                            <td class="text-center">
                                                <a href="{{ route('property_states.edit',['id' => $item->id, 'lang' => env('DEFAULT_LANGUAGE')]) }}" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">
                                                <span class="svg-icon svg-icon-3"><i class="text-primary fas fa-marker" ></i></span></a>
                                                
                                                <a data-href="{{ route('property_states.destroy', $item->id) }}"
                                                    class="confirm-delete btn btn-icon btn-bg-light btn-active-color-primary btn-sm"
                                                    data-href="{{ route('property_states.destroy', $item->id) }}">
...
@section('script')
    <script type="text/javascript">
          $("table").delegate(".onStatuChange", "change", function(){   
                ...
                let id = $(this).val();
                $.get("{{route('property_states.bulk')}}",
                { 
                    idz:id,
                    action:'status',
