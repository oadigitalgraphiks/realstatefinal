        ...
        <div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
            <div data-kt-swapper="true" data-kt-swapper-mode="prepend"
                ...
                <ul class="breadcrumb breadcrumb-separatorless fw-bold fs-7 my-1">
                    ...
                    <li class="breadcrumb-item text-muted"><a class="text-muted" href="{{route('property_countries.index')}}" >{{translate('Countries')}}</a></li>
                </ul>
            </div>
        </div>
    </div>
                    ...
                    <div class="card-header align-items-center py-5 gap-2 gap-md-5">
                        <div class="card-title">
                            ...
                            <div class="d-flex align-items-center position-relative my-1">
                                ...
                                <input type="text" class="form-control form-control-solid w-250px ps-14" id="sort_country" name="sort_country" @isset($sort_country) value="{{ $sort_country }}" @endisset placeholder="{{ translate('Type country name') }}" />
                            </div>
                          </form>
                        </div>
                        <div class="card-toolbar flex-row-fluid justify-content-end gap-5">
                            ...
                            <a href="{{ route('property_countries.create') }}" class="btn btn-primary" type="submit">{{ translate('Create New') }}</a>
                        </div>
                    </div>
                                ...
                                <tbody class="fw-bold text-gray-600">
                                    @foreach ($data as $key => $item)
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                <a href="{{ route('property_countries.edit',['id' => $item->id, 'lang' => env('DEFAULT_LANGUAGE')]) }}" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">
                                                <span class="svg-icon svg-icon-3"><i class="text-primary fas fa-marker" ></i></span></a>
                                                
                                                <a data-href="{{ route('property_countries.destroy', $item->id) }}"
                                                    class="confirm-delete btn btn-icon btn-bg-light btn-active-color-primary btn-sm"
                                                    data-href="{{ route('property_countries.destroy', $item->id) }}">
...
@section('script')
    <script type="text/javascript">
          $("table").delegate(".onStatuChange", "change", function(){   
                ...
                let id = $(this).val();
                $.get("{{route('property_countries.bulk')}}",
                { 
                    idz:id,
                    action:'status',
