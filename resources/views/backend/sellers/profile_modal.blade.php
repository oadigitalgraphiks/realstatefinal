<div class="modal fade" id="profile{{$seller->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
            <div class="text-center">
                ...
                <h1 class="h5 mb-1 pt-2">{{ $seller->name }}</h1>
            ...
            <div class="d-flex flex-column" >
                <span><span class=" text-dark">{{ translate('Phone')}}</span> : {{ $seller->phone ?? 'none'}}</span>
                <span><span class="text-dark" >{{ translate('Country')}}</span> : {{$seller->country ?? 'none'}}</span>
                <span><span class="text-dark" >{{ translate('State / Provence')}}</span> : {{ $seller->state ?? 'none'}}</span>
                <span><span class="text-dark" >{{ translate('City')}}</span> : {{ $seller->city ?? 'none'}}</span>
                <span><span class="text-dark">{{ translate('Zip Code')}}</span> : {{ $seller->postal_code ?? 'none'}}</span>
                <span><span class="text-dark">{{ translate('Street Addres')}}</span> : {{ $seller->address ?? 'none'}}</span>
            </div>
        </div>
      </div>
    </div>
