        ...
        <div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
            ...
            <div data-kt-swapper="true" data-kt-swapper-mode="prepend"
                ...
                <ul class="breadcrumb breadcrumb-separatorless fw-bold fs-7 my-1">
                    ...
                    <!--end::Item-->
                    <!--begin::Item-->
                    <li class="breadcrumb-item text-dark">Staff Edit</li>
                    <!--end::Item-->
                </ul>
                <!--end::Breadcrumb-->
            </div>
                            ...
                            <div class="card-body">
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-9 mb-5">
                                        <input type="text" placeholder="{{translate('Name')}}" value="{{ $staff->user->name }}" id="name" name="name" class="form-control" required>
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <label class="col-sm-3 col-from-label" for="email">{{translate('Email')}}</label>
                                    <div class="col-sm-9 mb-5">
                                        <input type="text" placeholder="{{translate('Email')}}" value="{{ $staff->user->email }}" id="email" name="email" class="form-control" required>
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <label class="col-sm-3 col-from-label" for="mobile">{{translate('Phone')}}</label>
                                    <div class="col-sm-9 mb-5">
                                        <input type="text" placeholder="{{translate('Phone')}}" value="{{ $staff->user->phone }}" id="mobile" name="mobile" class="form-control" required>
                                    </div>
                                </div>
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-9 mb-5">
                                        <select class="form-select mb-2" data-control="select2" data-hide-search="false" data-placeholder="Select an option" name="role_id" data-live-search="true">
                                            @foreach($roles as $role)
                                                <option value="{{$role->id}}" {{$staff->role_id == $role->id ? "selected" : ''}}>{{$role->getTranslation('name')}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
